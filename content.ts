...
(function () {
  ...
  async function contentIsModelAvailable(
    model: string,
    apiKeys: {
      openaiApiKey: string;
      geminiApiKey: string;
  ...
  function createOrUpdateSummaryDiv(
    ...
    `;
      ...
      chrome.storage.sync.get(
        ...
        async (result) => {
          ...
          for (const option of modelOptions) {
            ...
            const isAvailable = await contentIsModelAvailable(
              option.value,
              apiKeys
            );
            if (isAvailable) {
      ...
      modelSelect.addEventListener('change', async () => {
        ...
        const apiKeys = {
          openaiApiKey: (apiKeysResult as any).openaiApiKey || '',
          geminiApiKey: (apiKeysResult as any).geminiApiKey || '',
          anthropicApiKey: (apiKeysResult as any).anthropicApiKey || '',
        };
